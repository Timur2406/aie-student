...
def plot_problematic_columns(
    missing_df: pd.DataFrame,
    high_missing_threshold: float = 0.5,
    out_path: PathLike = "problematic_columns.png",
) -> Path:
    """
    Визуализация проблемных колонок с пропусками выше порога.
    """
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if missing_df.empty:
        ...
        # Фильтруем колонки с пропусками выше порога.
        problematic = missing_df[missing_df["missing_share"] > high_missing_threshold]
        ...
        if problematic.empty:
            ...
            ax.text(
                0.5,
                0.5,
                f"No columns with missing share > {high_missing_threshold:.0%}",
                ha="center",
                va="center",
            )
            ax.axis("off")
        else:
            fig, ax = plt.subplots(figsize=(10, max(4, len(problematic) * 0.4)))

            y_pos = np.arange(len(problematic))
            ax.barh(y_pos, problematic["missing_share"].values * 100)
            ax.set_yticks(y_pos)
            ax.set_yticklabels(problematic.index)
            ax.set_xlabel("Missing share (%)")
            ax.set_title(f"Columns with missing share > {high_missing_threshold:.0%}")

            # Добавляем значения на столбцы.
            for i, v in enumerate(problematic["missing_share"].values):
                ax.text(v * 100 + 0.5, i, f"{v:.1%}", va="center")

            ax.axvline(
                x=high_missing_threshold * 100, color="red", linestyle="--", alpha=0.5
            )
